...
class OperationalHubContractTests(unittest.TestCase):
    ...
    def test_reasoning_is_persistent_and_collapsible_on_both_clients(self):
        ...
        self.assertIn('new AssistantActivityRecord("reasoning"', windows)
        self.assertIn("CollapseActivityOnce()", windows)
        self.assertIn("HermesActivityDisclosure(timeline)", android_ui)
        self.assertIn("activityTimeline", android_stream)
        ...
        self.assertIn("ActivityTimeline", windows_archive)
        self.assertIn("Thinking = finalThinkingBuilder.ToString()", windows_home)
        self.assertIn("ActivityTimeline = bubble.ActivityTimeline.ToList()", windows_home)
        self.assertIn("RenderActivityDisclosure(timeline", windows_home)
        self.assertIn("thinking = message.Thinking", windows_gateway)
        self.assertIn("ActivityTimeline = ReadMessageActivityTimeline(message)", windows_gateway)
