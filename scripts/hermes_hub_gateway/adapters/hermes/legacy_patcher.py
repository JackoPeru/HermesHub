...
'''
    ...
    conversation_get = r'''    async def _handle_get_hub_conversations(self, request: "web.Request") -> "web.Response":
        auth_error = self._check_auth(request)
        _hermes_hub_bind_request_context(request)
        if auth_error is not None:
            return auth_error
        try:
            loop = asyncio.get_running_loop()
...
def _patch_dynamic_http_routes(text: str) -> tuple[str, bool]:
    ...
    block = r'''        # HERMES_HUB_DYNAMIC_ROUTES_BEGIN
        routes.extend([
            ...
            ("GET", "/v1/hub/sync", self._handle_get_hub_sync),
...
def _patch_sqlite_sync_v1(text: str) -> tuple[str, list[str]]:
    """Wire the modular SQLite/sync runtime into the generated upstream app.

    The generated code only performs composition and compatibility delegation;
    migrations, LWW, tombstones, and revision serialization live in the
...
def _hermes_hub_gateway_runtime_store():
    ...
    if "_hermes_hub_sqlite_runtime_store" in globals():
        return _hermes_hub_sqlite_runtime_store
    import importlib.util as _importlib_util
    import sys as _sys
    from pathlib import Path as _Path

    roots = []
    configured = os.environ.get("HERMES_HUB_GATEWAY_PACKAGE", "").strip()
    if configured:
    ...
    # Package presence is authoritative. Import/init/migration failures must
    # propagate so callers fail closed instead of silently returning JSON data.
    from hermes_hub_gateway.infrastructure.runtime_store import HubRuntimeStore

    _hermes_hub_sqlite_runtime_store = HubRuntimeStore.from_environment()
    return _hermes_hub_sqlite_runtime_store


def _hermes_hub_sqlite_source(env_name, default_name):
    return _hermes_hub_storage_path(env_name, default_name)


def _hermes_hub_request_context(request=None):
    try:
        import contextvars as _contextvars
        from hermes_hub_gateway.modules.protocol import CorrelationContext, new_correlation_context
        if "_hermes_hub_request_context_var" not in globals():
            globals()["_hermes_hub_request_context_var"] = _contextvars.ContextVar(
                "hermes_hub_request_context", default=None
            )
        current = globals()["_hermes_hub_request_context_var"].get()
        if current is not None:
            return current
        if request is not None:
            request_id = request.headers.get("X-Hermes-Request-Id") or request.headers.get("X-Request-Id")
            correlation_id = request.headers.get("X-Hermes-Correlation-Id") or request_id
            if request_id and correlation_id:
                value = CorrelationContext(str(request_id).strip(), str(correlation_id).strip())
                globals()["_hermes_hub_request_context_var"].set(value)
                return value
...
def _hermes_hub_bind_request_context(request):
    _hermes_hub_request_context(request)


def _hermes_hub_sync_payload(since, limit=500):
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is None:
        return {
    ...
    return runtime.changes_since(since, limit)


_hermes_hub_legacy_conversations_payload = _hermes_hub_conversations_payload
_hermes_hub_legacy_merge_conversations = _hermes_hub_merge_conversations
_hermes_hub_legacy_delete_conversation = _hermes_hub_delete_conversation
_hermes_hub_legacy_state_payload = _hermes_hub_state_payload
_hermes_hub_legacy_add_state = _hermes_hub_add_state
_hermes_hub_legacy_delete_state = _hermes_hub_delete_state
_hermes_hub_legacy_event_payload = _hermes_hub_conversation_event_payload


def _hermes_hub_conversations_payload():
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is not None:
        return runtime.conversations_payload(
            _hermes_hub_sqlite_source("HERMES_HUB_CONVERSATIONS_PATH", "hub_conversations.json")
        )
    return _hermes_hub_legacy_conversations_payload()


def _hermes_hub_merge_conversations(incoming):
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is not None:
        values = incoming if isinstance(incoming, list) else []
        return runtime.merge_conversations(
            _hermes_hub_sqlite_source("HERMES_HUB_CONVERSATIONS_PATH", "hub_conversations.json"),
            values,
        )
    return _hermes_hub_legacy_merge_conversations(incoming)


def _hermes_hub_delete_conversation(conversation_id):
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is not None:
        return runtime.delete_conversation(
            _hermes_hub_sqlite_source("HERMES_HUB_CONVERSATIONS_PATH", "hub_conversations.json"),
            conversation_id,
        )
    return _hermes_hub_legacy_delete_conversation(conversation_id)


def _hermes_hub_state_payload():
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is not None:
        return runtime.state_payload(_hermes_hub_sqlite_source("HERMES_HUB_STATE_PATH", "hub_state.json"))
    return _hermes_hub_legacy_state_payload()


def _hermes_hub_add_state(payload):
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is not None:
        return runtime.add_state(
            _hermes_hub_sqlite_source("HERMES_HUB_STATE_PATH", "hub_state.json"),
            payload if isinstance(payload, dict) else {},
        )
    return _hermes_hub_legacy_add_state(payload)


def _hermes_hub_delete_state(state_id):
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is not None:
        return runtime.delete_state(
            _hermes_hub_sqlite_source("HERMES_HUB_STATE_PATH", "hub_state.json"),
            state_id,
        )
    return _hermes_hub_legacy_delete_state(state_id)


def _hermes_hub_conversation_event_payload(reason, result=None):
    base = _hermes_hub_legacy_event_payload(reason, result)
    runtime = _hermes_hub_gateway_runtime_store()
    if runtime is None:
        return base
    try:
        from hermes_hub_gateway.modules.protocol import build_event_envelope
        revision = int((result or {}).get("revision") or runtime.sqlite.current_revision())
        envelope = build_event_envelope(
            event_type="hub.conversations.updated",
            payload=dict(base),
            sequence=revision,
            context=_hermes_hub_request_context(),
...
'''
    ...
    handler = '''    async def _handle_get_hub_sync(self, request: "web.Request") -> "web.Response":
        ...
        _hermes_hub_bind_request_context(request)
        try:
            since = int(request.query.get("since", request.query.get("revision", "0")) or 0)
        ...
        if since < 0 or limit < 1:
            return web.json_response({"error": "since must be non-negative and limit must be positive"}, status=400)
        try:
            return web.json_response(_hermes_hub_sync_payload(since, limit))
        except Exception as exc:
            return web.json_response({"error": "Hub sync unavailable", "detail": str(exc)[:240]}, status=503)

'''
    handler_anchor = '    async def _handle_models(self, request: "web.Request") -> "web.Response":'
    if "async def _handle_get_hub_sync" not in text:
        text, _ = _replace_once(text, handler_anchor, handler + handler_anchor, "Hub sync endpoint handler")
        changes.append("Hub sync endpoint handler")

    if '"hub_sync": True,' not in text:
        if '                "hub_state": True,\n' in text:
            text = text.replace(
                '                "hub_state": True,\n',
                '                "hub_state": True,\n                "hub_sync": True,\n',
    ...
    if '"hub_sync": {"method": "GET", "path": "/v1/hub/sync?since=revision"}' not in text:
        ...
        text, _ = _replace_regex_once(
            ...
            hub_state_endpoint,
            r'\1'
            r'                "hub_sync": {"method": "GET", "path": "/v1/hub/sync?since=revision"},' "\n",
            "capabilities Hub sync endpoint",
        )
        changes.append("capabilities Hub sync endpoint")

    if not _route_registered(text, "GET", "/v1/hub/sync", "_handle_get_hub_sync"):
        state_route = r'(^\s+self\._app\.router\.add_get\("/v1/hub/state", self\._handle_get_hub_state\)\n)'
        text, _ = _replace_regex_once(
            text,
            state_route,
            r'\1'
            r'            self._app.router.add_get("/v1/hub/sync", self._handle_get_hub_sync)' "\n",
            "router Hub sync endpoint",
        )
        changes.append("router Hub sync endpoint")

    for handler_name in (
        "_handle_get_hub_conversations",
        "_handle_get_hub_conversations_events",
        "_handle_put_hub_conversation",
        "_handle_post_hub_conversation",
        "_handle_post_hub_conversations_import",
        "_handle_delete_hub_conversation",
        "_handle_get_hub_state",
        "_handle_post_hub_state",
        "_handle_delete_hub_state",
    ):
        definition = f'    async def {handler_name}'
        start = text.find(definition)
        if start < 0:
            continue
        end = text.find("\n    async def ", start + len(definition))
        if end < 0:
            end = len(text)
        segment = text[start:end]
        if "_hermes_hub_bind_request_context(request)" in segment:
            continue
        auth_line = "        auth_error = self._check_auth(request)\n"
        if auth_line in segment:
            segment = segment.replace(auth_line, auth_line + "        _hermes_hub_bind_request_context(request)\n", 1)
...
'''
    ...
    unlimited_request_limit_block = '_HERMES_GATEWAY_MAX_REQUEST_MB = int(os.environ.get("HERMES_GATEWAY_MAX_REQUEST_MB", "0"))\nMAX_REQUEST_BYTES = (_HERMES_GATEWAY_MAX_REQUEST_MB if _HERMES_GATEWAY_MAX_REQUEST_MB > 0 else 102400) * 1024 * 1024  # 0 maps to a 100GB practical ceiling for aiohttp'
    finite_request_limits = (
    ...
    for finite_request_limit in finite_request_limits:
        if finite_request_limit in text:
            text = text.replace(finite_request_limit, unlimited_request_limit_block, 1)
            changes.append("unlimited gateway request default")
...
def _hermes_hub_transcode_mp4(source: "Path") -> "Path":
    ...
    text, sqlite_sync_changes = _patch_sqlite_sync_v1(text)
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(1)
def _patch_agent_runtime_and_correlation_v1(text: str) -> tuple[str, list[str]]:
    """Compose correlation and AgentRuntime around unchanged upstream logic."""

    changes: list[str] = []
    if _AGENT_RUNTIME_MARKER not in text:
        legacy_method = "    async def _run_agent(\n"
        if legacy_method not in text:
            raise PatchError("Hermes Agent _run_agent anchor not found")
        text = text.replace(
            legacy_method,
            "    async def _hermes_hub_legacy_run_agent(\n",
            1,
        )
        wrapper_anchor = "\n    # ------------------------------------------------------------------\n    # /v1/runs"
    ...
    async def _run_agent(
        ...
        request = AgentRunRequest(
            ...
            prompt=user_message,
            model=str((route or {}).get("model") or getattr(self, "_model_name", "hermes-agent")),
            preferred_protocol="hermes-native",
        ...
        if isinstance(adapter_result.result, tuple) and len(adapter_result.result) == 2:
            return adapter_result.result
        return (
            {"final_response": adapter_result.text},
            dict(adapter_result.usage or {}),
        )
'''
        if wrapper_anchor not in text:
            raise PatchError("Hermes Agent runtime wrapper anchor not found")
        text = text.replace(wrapper_anchor, wrapper + wrapper_anchor, 1)
        changes.append("AgentRuntime adapter composition over upstream agent loop")

    if _CORRELATION_RUNTIME_MARKER not in text:
        runtime = r'''# HERMES_HUB_CORRELATION_RUNTIME_V1
def _hermes_hub_request_context(request=None):
    from hermes_hub_gateway.adapters.hermes.correlation_runtime import request_context
    return request_context(request)


def _hermes_hub_bind_request_context(request):
    from hermes_hub_gateway.adapters.hermes.correlation_runtime import bind_request_context
    return bind_request_context(request)


def _hermes_hub_agent_runtime_adapter(owner):
    from hermes_hub_gateway.adapters.hermes.agent_runtime import adapter_for_legacy_owner
    return adapter_for_legacy_owner(owner)


async def _hermes_hub_correlation_middleware(request, handler):
    from hermes_hub_gateway.adapters.hermes.correlation_runtime import correlation_middleware
    return await correlation_middleware(request, handler)


def _hermes_hub_install_correlation_runtime():
    from hermes_hub_gateway.adapters.hermes.correlation_runtime import install
    return install(web)
'''
        text += "\n\n" + runtime.rstrip() + "\n"
        changes.append("request/correlation bridge and canonical SSE envelope runtime")

    app_anchor = "            self._app = web.Application(middlewares=mws, client_max_size=MAX_REQUEST_BYTES)\n"
    middleware_line = "            mws.append(web.middleware(_hermes_hub_correlation_middleware))\n"
    if middleware_line not in text and app_anchor in text:
        text = text.replace(
            app_anchor,
            "            _hermes_hub_install_correlation_runtime()\n"
            + middleware_line
            + app_anchor,
            1,
        )
        changes.append("correlation middleware composition")
    return text, changes
